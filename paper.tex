...
We call this general method \emph{Typed Domain Grounding} (TDG): rather than anchoring an individual answer in retrieved facts --- the data-space grounding of retrieval-augmented generation --- TDG anchors an entire domain language in syntax and type knowledge the model already possesses. The design principle is: \emph{never invent syntax an LLM has never seen; embed the domain in a language it already knows, and let the compiler be the oracle.} Host-language choice is therefore not an implementation detail but an explicit criterion --- Kotlin is selected in part for its massive presence in LLM training corpora, so that a model asked to write kUML reuses grammar it has seen at scale and only has to learn the domain's vocabulary, every element of which the compiler can check. TDG and RAG-style grounding are orthogonal and composable; this paper evaluates the language-space instance empirically.
...
Rendered with \texttt{kuml render}, the listing above produces Figure~\ref{fig:gcr-loop} --- the same source that describes the loop in prose also draws it, a small illustration of the paper's own thesis: a typed model is not just checkable, it is directly renderable.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\linewidth]{figures/gcr-loop.pdf}
\caption{The Generate-Compile-Repair loop, rendered by kUML from the kUML source listing above.}
\label{fig:gcr-loop}
\end{figure}
...
\texttt{kuml render} turns the listing above directly into Figure~\ref{fig:banking} --- no manual layout step, no separate diagramming tool. This is the deliverable the compiler oracle protects: a checked model that is also a picture.

\begin{figure}[h]
\centering
\includegraphics[width=0.7\linewidth]{figures/banking.pdf}
\caption{The \texttt{Banking} class diagram, rendered by kUML from the kUML source listing above.}
\label{fig:banking}
\end{figure}
...
\addplot[fill=black!75] coordinates {(SF,45.3) (HR,15.4)};
\addplot[fill=black!40] coordinates {(SF,37.9) (HR,16.5)};
\addplot[fill=black!15] coordinates {(SF,37.4) (HR,21.9)};
\legend{kUML + GCR, PlantUML (+R), Mermaid (+R)}
\end{axis}
\end{tikzpicture}
\caption{Semantic fidelity (higher is better) and hallucination rate (lower is better), aggregated across all 50 tasks, single-model run (Claude Sonnet 5). kUML leads on both quality metrics despite the permissive baselines' higher compile rates (Table~\ref{tab:overall}).}
\label{fig:sf-hr-bars}
\end{figure}

The pattern splits cleanly in two. On the syntactic metrics, the permissive parsers accept nearly everything on the first shot (CR = 98.0\%); kUML starts lower, as expected (78.0\%), and is lifted by the repair loop to near-baseline level (FCR = 98.0\%). On the quality-oriented metrics the picture inverts: kUML delivers the highest semantic fidelity (45.3\% vs.\ 37.9\% and 37.4\%) and the lowest hallucination rate (15.4\% vs.\ 16.5\% and 21.9\%), visualized in Figure~\ref{fig:sf-hr-bars}.
...
The benchmark corpus, gold-standard models, harness, and kUML itself are released as open source,\footnote{Reproducibility package and methodology: \url{https://docs.kuml.dev/kuml/reference/llm-benchmark.html}. Source: \url{https://github.com/kuml-dev/kUML}.} and we invite the community to extend the corpus with additional diagram families, target DSLs, and models. The three-model full study remains outstanding; the evidence so far, however, already supports a simple practical recommendation: when LLMs write your models, let a compiler read them first.
